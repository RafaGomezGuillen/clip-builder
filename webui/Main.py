...
SETTINGS_PRESET_SCHEMA = "clip-builder.settings-preset"
SETTINGS_PRESET_VERSION = 1
SETTINGS_PRESET_FILE_NAME = "clip-builder-settings.json"
KEY_BACKUP_SCHEMA = "clip-builder.key-backup"
KEY_BACKUP_VERSION = 1
KEY_BACKUP_FILE_NAME = "clip-builder-keys.json"
...
def _apply_pending_task_restore():
    ...
    _apply_restored_params(payload["params"], payload.get("webui", {}))
    st.session_state["task_restore_succeeded"] = True
    logger.info(f"restored task configuration: {payload['task_id']}")
    return True


def _apply_restored_params(params, restore_webui=None):
    """
    把一份完整的生成参数写回页面控件状态。
    ...
    restore_webui = restore_webui or {}
...
def _render_local_script_generation(params):
    """保留 clip-builder 原有的本地 LLM 脚本生成路径。"""
    if not st.button(
...
def _render_video_settings(panel, params):
    ...
    with panel:
        with st.container(border=True):
            ...
            video_sources_widget_key = localized_widget_key("video_sources_select")
            multiselect_args = {
                "label": tr("Video Source"),
                "options": [value for _, value in video_sources],
                "key": video_sources_widget_key,
                "format_func": lambda value: source_labels[value],
            }
            if video_sources_widget_key not in st.session_state:
                multiselect_args["default"] = [
                    source for source in saved_video_sources if source in source_labels
                ]
            params.video_sources = st.multiselect(**multiselect_args)
            ...
            # Coverr 的素材大多是 16:9 横屏,默认竖屏会让画面被大量黑边包围。
            ...
            default_aspect_index = 1 if primary_source in {"coverr"} else 0
            video_aspect_values = [value for _, value in video_aspect_ratios]
            video_aspect_config_key = f"video_aspect_{primary_source}"
...
def _render_generation_controls(
    ...
    if start_button:
        ...
        if not params.video_sources or not set(params.video_sources).issubset(
            valid_video_sources
        ):
            _remove_active_generation_task(task_id)
